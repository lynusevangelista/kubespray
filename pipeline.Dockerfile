...
RUN update-alternatives --install /usr/bin/python python /usr/bin/python3 1 \
    ...
    && curl -L https://dl.k8s.io/release/v1.34.8/bin/linux/$(dpkg --print-architecture)/kubectl -o /usr/local/bin/kubectl \
    && echo $(curl -L https://dl.k8s.io/release/v1.34.8/bin/linux/$(dpkg --print-architecture)/kubectl.sha256) /usr/local/bin/kubectl | sha256sum --check \
